  ...
  return (
    <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-sm border border-neutral-200 dark:border-neutral-800 p-4 sm:p-5 3xl:p-8 transition-colors animate-fade-in">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3.5 sm:gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 3xl:gap-3">
          {/* Mode Switcher */}
          <div className="flex items-center bg-neutral-100/90 dark:bg-neutral-800 p-1 3xl:p-1.5 rounded-xl self-start sm:self-auto">
            <button
              onClick={() => onFilterModeChange('single')}
              className={`flex items-center gap-1.5 px-3 py-1.5 3xl:px-4 3xl:py-2 rounded-lg text-xs 3xl:text-base font-medium transition-all cursor-pointer ${
                filterMode === 'single'
                  ? 'bg-white dark:bg-neutral-950 text-primary shadow-xs font-semibold'
                  : 'text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white'
              }`}
            >
              <CalendarDays className="w-3.5 h-3.5 3xl:w-4 3xl:h-4" />
            ...
            >
              <CalendarRange className="w-3.5 h-3.5 3xl:w-4 3xl:h-4" />
              <span>ช่วงวันที่</span>
            </button>
          </div>

          {/* Quick Presets */}
          <div className="flex items-center gap-1.5 3xl:gap-2 bg-neutral-100/80 dark:bg-neutral-800 p-1.5 3xl:p-2 rounded-xl">
            {(filterMode === 'single' ? SINGLE_PRESETS : RANGE_PRESETS).map((preset) => (
              <button
                key={preset.key}
                onClick={() => onPresetChange(preset.key)}
                className={`px-3 py-1.5 3xl:px-5 3xl:py-2 rounded-lg text-xs 3xl:text-base font-medium transition-all text-center cursor-pointer hover:scale-[1.02] active:scale-95 ${
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        {filterMode === 'single' ? (
          /* Single Date Mode */
          <div className="flex items-center gap-2 bg-neutral-50 dark:bg-neutral-950 px-3.5 py-2 sm:py-1.5 3xl:px-5 3xl:py-3 rounded-xl border border-neutral-200 dark:border-neutral-800 w-full sm:w-auto">
            <span className="text-xs 3xl:text-base font-medium text-neutral-500 dark:text-neutral-400 shrink-0">
              เลือกวันที่:
            </span>
            <input
              type="date"
